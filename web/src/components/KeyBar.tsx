...
const BTN = "min-w-9 h-9 px-2 rounded border border-border bg-muted/60 text-foreground text-xs " +
  "font-mono leading-none flex items-center justify-center active:bg-muted select-none";
 ...
 */
export function KeyBar({ onKey, applicationCursorKeys, onCtrlArmedChange, ctrlArmed }: Props): JSX.Element | null {
  // Pointer, not width: a tablet in landscape is wide and still has no arrow keys, and a narrow
  // desktop window has both. `matchMedia` is read in an effect so the first server-free render is
  // stable and a device that changes pointer (a tablet gaining a keyboard) re-evaluates.
  const [coarse, setCoarse] = useState(false);
  useEffect(() => {
    // Feature-detected exactly like ThemeProvider does: matchMedia is absent under jsdom, and a bar
    // that throws would take the whole modal down with it. Absent means "assume a real pointer" —
  ...
  return (
    ...
    <div className="shrink-0 flex items-center gap-1 px-1 py-1 border-t border-border overflow-x-auto">
      ...
      <button
        ...
        onPointerDown={(e) => { e.preventDefault(); onCtrlArmedChange(!ctrlArmed); }}
      >ctrl</button>
      {ARROW_KEYS.map((a) => (
        <button
          key={a.id}
